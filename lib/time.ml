...
let sleep_ns d =
  ...
  Logs.info (fun f -> f "sleep_ns : org_time: %s" (Int64.to_string Monotonic.(time() + of_nanoseconds 0L)));
  ...
  let sleeper = { time = d; canceled = false; thread = w } in
...
let timeout d = Logs.info (fun f -> f "sleep_ns : d: %s" (Int64.to_string d)); sleep_ns d >>= fun () -> Lwt.fail Timeout
